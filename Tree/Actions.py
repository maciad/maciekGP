...
class Action(Node):
    ...
    @staticmethod
    def new_action(ctx):
        return NodeFactory.get_new_action(ctx.config, ctx)

    def full_grow(self, ctx, target_depth):
        pass


class BlockStatement(Action, IMutable, IGrowable):
    ...
    @property
    def actions(self):
        return [c for c in self.children if isinstance(c, Action)]

    @property
    def growables(self):
        return [c for c in self.children if isinstance(c, Action) and isinstance(c, IGrowable)]

    def add(self, action):
        self.children.append(action)
        # action.parent = self
        # action.update_parents()
    ...
    def grow(self, ctx):
        self.add(Action.new_action(ctx))
    ...
    def full_grow(self, ctx, target_depth):
        ...
        for action in self.actions:
            action.full_grow(ctx, target_depth - 1)
...
class Assignment(Action, IGrowable):
